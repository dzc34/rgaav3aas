...
var pushMessage = function(messages, messageId, placeholders) {
  if(messageId && i18n[messageId]) {
    messages.push(format(i18n[messageId], placeholders));
  } else {
    messages.push('Aucun message avec la clef ' + messageId);
  }
};
...
Rule.prototype.addfailonelement = function(fn, messageId) {
    this.failonelement.push({
        'fn': fn,
        'messageId': messageId
    });
...
Rule.prototype.withattribute = function(attribute, messageId) {
    this.failonelement.push({
        'fn': function(element, $) {
            return $(element).attr(attribute) !== undefined && $(element).attr(attribute) !== null;
        },
        'messageId': messageId
    });
...
Rule.prototype.withelement = function(tag, messageId) {
    this.failonelement.push({
        'fn': function(element, $) {
            return $(element).select(tag).length > 0;
        },
        'messageId': messageId
    });
    ...
    elements.each(function() {
        ...
        if (rule.failonelement) {
            ...
            rule.failonelement.forEach(function(check) {
                if (!check.fn(element, $)) {
                  pushMessage(messages, check.messageId || rule.test, [placeholders]);
                }
            });
        }
